...
public class TrailRendererWith2DCollider : MonoBehaviour {
	...
	public void ChangeTrailMaterial(Material material) {
		...
		foreach (TrailPath trail in trails) 
		{
			trail.collider.GetComponent<Renderer>().material = material;
		}
	}
	...
	public void ChangeColliderTrigger(bool isTrigger) {
		...
		foreach (TrailPath trail in trails) 
		{
			trail.collider.isTrigger = isTrigger;
		}
	}
	...
	public void ChangeColliderEnabled(bool enabled) {
		...
		foreach (TrailPath trail in trails) 
		{
			trail.collider.enabled = enabled;
		}
	}

	public void CutOff()
	{
		if (trails.Count > 0) trails[0].isFinished = true;
		trails.Insert (0, new TrailPath (this));
	}
	
	public void ClearUndrawnTrails()
	{
		for (int i = 0; i < trails.Count; i++)
		{
			if ((Time.time - trails[i].maxTime) > lifeTime)
			{
	...
	private void Awake() {
		...
		trails = new List<TrailPath>();
		//trails.Add(new TrailPath(this));
	}
	
	private void Update() {
		if (!pausing && trails.Count > 0) {
	...
	private bool TryAddVertices() {
		...
		if ( !trails[0].isFinished && (trails[0].centerPositions.First.Value - trans.position).sqrMagnitude > vertexDistanceMin * vertexDistanceMin) {
			//calculate the normalized direction from the 1) most recent position of vertex creation to the 2) current position
			Vector3 dirToCurrentPos = (trans.position - trails[0].centerPositions.First.Value).normalized; 
			...
			trails[0].leftVertices.AddFirst(new Vertex(leftPos, trans.position, (leftPos - trans.position).normalized) );
			trails[0].rightVertices.AddFirst(new Vertex(rightPos, trans.position, (rightPos - trans.position).normalized) );
			
			//add the current position as the most recent center position
			trails[0].centerPositions.AddFirst(trans.position);
			trails[0].maxTime = Time.time;
			vertsAdded = true;
		}
	...
	private bool TryRemoveVertices() {
		...
		for (int i = 0; i < trails.Count; i++) {
			...
			LinkedListNode<Vertex> leftVertNode = trails[i].leftVertices.Last;
	...
	private void SetVertexWidths() {
		for (int i = 0; i < trails.Count; i++)
		{
			LinkedListNode<Vertex> leftVertNode = trails[i].leftVertices.First;
			LinkedListNode<Vertex> rightVertNode = trails[i].rightVertices.First;
	...
	private void SetMesh() {
		for (int i = 0; i < trails.Count; i++)
		{
			...
			if (trails[i].centerPositions.Count < 2) {
				continue;
			}
			
			//create an array for the 1) trail vertices, 2) trail uvs, 3) trail triangles, and 4) vertices on the collider path
			Vector3[] vertices = new Vector3[trails[i].centerPositions.Count * 2];
			Vector2[] uvs = new Vector2[trails[i].centerPositions.Count * 2];
			int[] triangles = new int[ (trails[i].centerPositions.Count - 1) * 6];
			Vector2[] colliderPath = new Vector2[ (trails[i].centerPositions.Count - 1) * 2];
			
			LinkedListNode<Vertex> leftVertNode = trails[i].leftVertices.First;
			LinkedListNode<Vertex> rightVertNode = trails[i].rightVertices.First;
			
			//get the change in time between the first and last pair of vertices
			float timeDelta = trails[i].leftVertices.Last.Value.TimeAlive - trails[i].leftVertices.First.Value.TimeAlive;

			float lastUVValue = 0;
			//iterate through all the pairs of vertices (left + right)
			for (int j = 0; j < trails[i].leftVertices.Count; ++j) {
				Vertex leftVert = leftVertNode.Value;
				Vertex rightVert = rightVertNode.Value;
			...
			trails[i].mesh.Clear();
			trails[i].mesh.vertices = vertices;
			trails[i].mesh.uv = uvs;
			trails[i].mesh.triangles = triangles;
			
			if (colliderEnabled) {
				trails[i].collider.SetPath(0, colliderPath);
			}
		}
	}
	...
	private class TrailPath {
		public GameObject gameObject;
		public Mesh mesh;                              
		public PolygonCollider2D collider;
		
		public LinkedList<Vector3> centerPositions;
		public LinkedList<Vertex> leftVertices;        
		public LinkedList<Vertex> rightVertices;

		public bool isFinished = false;
		public float maxTime;

		public TrailPath(TrailRendererWith2DCollider trailRenderer)
		{
			gameObject = new GameObject("Trail", new[] { typeof(MeshRenderer), typeof(MeshFilter), typeof(PolygonCollider2D) } );
			gameObject.layer = trailRenderer.gameObject.layer;
			mesh = gameObject.GetComponent<MeshFilter>().mesh = new Mesh();
			gameObject.GetComponent<Renderer>().material = trailRenderer.trailMaterial;
			
			//get and set the polygon collider on this trail.
			collider = gameObject.GetComponent<PolygonCollider2D>();
			collider.isTrigger = trailRenderer.colliderIsTrigger;
			collider.SetPath(0, null);
			
			//set the first center position as the current position
			centerPositions = new LinkedList<Vector3>();
			centerPositions.AddFirst(trailRenderer.trans.position);
